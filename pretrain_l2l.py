...
def get_indices():
    ...
    shot = 1
    query_num = 15
    support_indices = np.zeros(data.shape[0], dtype=bool)
...
if __name__ == '__main__':
    ...
    save_path1 = '-'.join([args.dataset, args.backbone_class, 'Pre_l2l_aug']) # Pre_l2l_aug
    ...
    if args.dataset == 'MiniImageNet':
        ...
        # dataset
        path_data = '/home/mayug/projects/few_shot/data/'
        trainset = l2l.vision.datasets.MiniImagenet(
        ...
        valset = l2l.vision.datasets.MiniImagenet(
            root=path_data, mode='validation', 
            transform=val_transforms
        )
    
        valset = l2l.data.MetaDataset(valset)

        valid_transforms = [
            NWays(valset, len(valset.labels)),
            KShots(valset, 1 + args.query),
            LoadData(valset),
            RemapLabels(valset),
        ]
        valid_tasks = l2l.data.TaskDataset(valset,
                                        task_transforms=valid_transforms,
                                        num_tasks=200)
    train_loader = DataLoader(dataset=trainset, batch_size=args.batch_size, shuffle=True, num_workers=8, pin_memory=True)
    args.num_class = len(trainset.labels)
    # val_sampler = CategoriesSampler([int(i) for i in valset.labels], 200, len(valset.labels), 1 + args.query) # test on 16-way 1-shot
    # val_loader = DataLoader(dataset=valset, batch_sampler=val_sampler, num_workers=8, pin_memory=True)
    val_loader = DataLoader(dataset=valid_tasks, num_workers=8, pin_memory=True)
    # print('results', next(iter(val_loader))[0].shape)
    # asd
    args.way = len(valset.labels)
    args.shot = 1
    ...
    for epoch in range(init_epoch, args.max_epoch + 1):
        ...
        for i, batch in enumerate(train_loader, 1):
            ...
            # np.save('./l2l_img.npy', data.cpu().numpy())
            # asd
            logits = model(data)
        ...
        if epoch > 100 or (epoch-1) % 5 == 0 or True:
            model.eval()
            vl_dist = Averager()
            va_dist = Averager()
            vl_sim = Averager()
            ...
            # label = torch.arange(len(valset.labels)).repeat(args.query)
            ...
            with torch.no_grad():
                for i, batch in tqdm(enumerate(val_loader, 1)):
                    ...
                    sort = torch.sort(label)
                    data = data.squeeze(0)[sort.indices].squeeze(0)
                    label = label.squeeze(0)[sort.indices].squeeze(0)
                    

                    support_indices, query_indices = get_indices()

                    data_shot, data_query = data[support_indices], data[query_indices] # 16-way test
                    # print ('label support ', label[support_indices])
                    label = label[query_indices]
                    ...
                    # print('data', [data.shape, data.min(), data.max()])
                    # np.save('./l2l_img_shot.npy', data_shot.cpu().numpy())
                    # np.save('./l2l_img_query.npy', data_query.cpu().numpy())
                    # print('label query', label)
                    logits_dist, logits_sim = model.forward_proto(data_shot, data_query, len(valset.labels))
                    # print('logits_dist', logits_dist.shape)
                    # asd
