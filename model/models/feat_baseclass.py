...
# def get_k_base(proto, all_proto, k=10):
...
#     indices = similarities_sorted.indices
#     return indices[:, :k]
...
class FEATBaseClass(FewShotModel):
    ...
    def add_base_classes(self, proto):
        base_wt = self.args.base_wt
        proto = proto.squeeze()
        top_k, wts = get_k_base(proto, self.all_proto, 
                                k=self.args.k, return_weights=True,
                                train=self.training)
    ...
    def _forward(self, instance_embs, support_idx, query_idx):
        ...
        proto = self.add_base_classes(proto)
