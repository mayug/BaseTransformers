...
if __name__ == '__main__':
    ...
    if args.resume == True:
        ...
        if args.init_weights:
            state = torch.load(args.init_weights)['params']
            state = {('encoder.'+k if 'FC' not in k else k.lower()):v for k,v in state.items()}
            # state['']
            # state = {k.lower():v for k,v in state.items() if 'fc' in k}
