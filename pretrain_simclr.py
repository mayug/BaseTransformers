...
def get_simclr_loss(data_simclr, model, temperature_simclr, 
    label_aux=None, fc_simclr=None):
    
    n_batch, n_views, _, input_spat, _ = data_simclr.shape
    data_simclr= data_simclr.reshape(-1, 3, input_spat, input_spat)
    # print('data_simclr after reshape', data_simclr.shape)
    
    # simclr_features = model(data_simclr)
    simclr_features = model.encoder(data_simclr)
    ...
    # print('after choose ', [logits_simclr.shape, logits_simclr.min(), logits_simclr.max()])
    if label_aux is None:
        print('calling inside')
        label_aux = get_label_aux(n_batch)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--return_simclr', type=int, default=None)
    parser.add_argument('--balance', type=float, default=0.1)
    parser.add_argument('--temperature_simclr', type=float, default=0.1)
    parser.add_argument('--simclr_fc', type=bool, default=False)
    ...
    save_path2 = '_'.join([str(args.lr), str(args.gamma), str(args.schedule), str(dt_string)])
    ...
    trainset = Dataset('train', args, augment=True, return_simclr=args.return_simclr)
    ...
    label_aux = get_label_aux(args.batch_size)

    # construct model
    model = Classifier(args)
    print('args.simclr_fc ', [args.simclr_fc, type(args.simclr_fc)])
    fc_simclr = None 
    if args.simclr_fc:
        dim_mlp = 64
        dim_out =64
        fc_simclr = nn.Sequential(nn.Linear(dim_mlp, dim_mlp), nn.ReLU(), nn.Linear(dim_mlp, dim_out))
        fc_simclr = fc_simclr.cuda()
    ...
    if args.resume == True:
        ...
        trlog['train_total_loss'] = []
        trlog['train_aux_loss'] = []
        trlog['train_primary_loss'] = []
        trlog['val_loss_dist'] = []
        trlog['val_loss_sim'] = []
        trlog['train_acc'] = []
        trlog['val_acc_sim'] = []
    ...
    for epoch in range(init_epoch, args.max_epoch + 1):
        ...
        for i, batch in enumerate(train_loader, 1):
            ...
            if torch.cuda.is_available():
                if args.return_simclr:
                    data, label, data_simclr= [_.cuda() for _ in batch]
                else:
                    data, label= [_.cuda() for _ in batch]
                label = label.type(torch.cuda.LongTensor)
            else:
                if args.return_simclr:
                    data, label, data_simclr = batch
                else:
                    data, label = batch
            ...
            if data_simclr is not None:
                # print('data_simclr shape ', data_simclr.shape)
                
                # print('data_simclr', [data_simclr.shape, data_simclr.min(), data_simclr.max()])
                aux_loss = get_simclr_loss(data_simclr, model, 
                    temperature_simclr=args.temperature_simclr, label_aux=label_aux,
                    fc_simclr=fc_simclr)
                total_loss = (1 - args.balance)*loss + args.balance*aux_loss 
                # asd
            acc = count_acc(logits, label)
            writer.add_scalar('data/loss', float(loss), global_count)
            writer.add_scalar('data/acc', float(acc), global_count)
            if (i-1) % 100 == 0:
                print('epoch {}, train {}/{}, total_loss={:.4f}, primary_loss={:.4f}, aux_loss={:.4f}, acc={:.4f}'.format(epoch, 
                    i, len(train_loader), total_loss.item(), loss.item(), aux_loss.item(), acc))

            tl.add(total_loss.item())
            tl_aux.add(aux_loss.item())
            tl_primary.add(loss.item())
            ta.add(acc)
        ...
        if epoch > 100 or (epoch-1) % 5 == 0:
            ...
            trlog['train_total_loss'].append(tl)
            trlog['train_primary_loss'].append(tl_aux)
            trlog['train_aux_loss'].append(tl_primary)
