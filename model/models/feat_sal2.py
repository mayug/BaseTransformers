...
class FEATSAL2(FewShotModel):
    def __init__(self, args):

        # so that output is b_s*n_c*h*w
        resize = True
        sal = False
        assert(args.sal_crop=='saliency_crop4')
        super().__init__(args, resize=resize, sal=sal)
    ...
    def _forward(self, instance_embs, support_idx, query_idx):
        # print(['instance_embs.shape ', instance_embs.shape])

        instance_embs = instance_embs*self.sal_embs
        # print('sal_emeds shape', self.sal_embs.shape)
        # print('instance_embs shape', instance_embs.shape)
